...
class OpenRouterAiConfigDefaultsTests(unittest.TestCase):
    def test_default_ai_config_uses_openrouter_gemini_3_pro_preview(self):
        config = ai_config.get_ai_config()

        self.assertEqual(config["provider_key"], "openrouter")
        self.assertEqual(config["model"], "google/gemini-3.1-pro-preview")
...
class ModelConfigPersistenceTests(unittest.TestCase):
    ...
    def test_explicit_yyds_selection_persists_after_reloading_configs(self):
        with self.Session() as db:
            ...
            self.assertEqual(config["provider_key"], "yyds")
            self.assertEqual(config["resolved_model_id"], "gemini-3.1-pro-high")
...
class StartupFunctionModelConfigMigrationTests(unittest.TestCase):
    ...
    def test_ensure_function_model_configs_rewrites_legacy_yyds_default_to_openrouter(self):
        ...
        try:
            ...
            self.assertEqual(row.provider_key, "openrouter")
            self.assertEqual(row.model_key, "google/gemini-3.1-pro-preview")
            self.assertEqual(row.model_id, "google/gemini-3.1-pro-preview")
        finally:
            db.close()
            engine.dispose()
