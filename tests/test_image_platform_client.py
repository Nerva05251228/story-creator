...
class ImagePlatformClientTests(unittest.TestCase):
    ...
    def test_fetch_requires_image_platform_configuration(self, mock_get):
        with self.assertRaisesRegex(RuntimeError, "IMAGE_PLATFORM_BASE_URL"):
            image_platform_client.fetch_image_model_catalog(timeout=12)
    ...
    @patch.dict(
        os.environ,
        {
            "IMAGE_PLATFORM_BASE_URL": "https://image.example.invalid/api/image",
            "IMAGE_PLATFORM_API_TOKEN": "test-image-token",
        },
        clear=True,
    )
    @patch("image_platform_client.requests.get")
    def test_fetch_filters_models_and_preserves_enabled_provider_fields(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            "https://image.example.invalid/api/image/models",
