...
def ensure_storyboard_columns():
    try:
        with engine.begin() as conn:
            ...
            if "reasoning_prompt_status" not in columns:
                conn.execute(
                    text("ALTER TABLE storyboard_shots ADD COLUMN reasoning_prompt_status TEXT DEFAULT 'idle'")
                )
                print("已添加 storyboard_shots.reasoning_prompt_status 字段")
                columns.add("reasoning_prompt_status")
            ...
            conn.execute(
                text("UPDATE storyboard_shots SET reasoning_prompt_status = 'idle' WHERE reasoning_prompt_status IS NULL OR TRIM(reasoning_prompt_status) = ''")
            )
            if (
...
请开始分析。"""
    ...
    {
        "key": STORYBOARD_REASONING_PROMPT_KEY,
        "name": "故事板推理提示词前缀",
        "description": "故事板（sora）生成推理提示词时使用的前缀模板，返回纯文本并回填到原剧本段落",
        "content": STORYBOARD_REASONING_PROMPT_DEFAULT
    },
    {
...
def handle_text_relay_task_success(db: Session, task: models.TextRelayTask, upstream_payload: Dict[str, Any]):
    ...
    if task_type == "storyboard_reasoning_prompt":
        shot_id = int(task_payload.get("shot_id") or task.owner_id or 0)
        shot = db.query(models.StoryboardShot).filter(models.StoryboardShot.id == shot_id).first()
        if not shot:
            return
        reasoning_text = _extract_text_relay_result_content(upstream_payload).strip()
        shot.script_excerpt = reasoning_text
        shot.reasoning_prompt_status = "completed"
        return
...
def handle_text_relay_task_failure(db: Session, task: models.TextRelayTask, upstream_payload: Dict[str, Any]):
    ...
    if task_type == "storyboard_reasoning_prompt":
        ...
        shot = db.query(models.StoryboardShot).filter(models.StoryboardShot.id == shot_id).first()
        if shot:
            shot.reasoning_prompt_status = "failed"
        return
...
VOICEOVER_TTS_API_URL = os.getenv("VOICEOVER_TTS_API_URL", "http://192.168.5.248:7860")
...
async def login(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    master_ok = request.password == MASTER_PASSWORD and request.username not in HIDDEN_USERS
...
async def verify_nerva_password(request: PasswordVerifyRequest):
    """验证nerva用户密码（硬编码）"""
    NERVA_PASSWORD = "10241024"

    if request.password == NERVA_PASSWORD:
        return {"success": True}
    else:
        raise HTTPException(status_code=401, detail="密码错误")
...
def _verify_admin_panel_password(x_admin_password: Optional[str]) -> None:
    if (x_admin_password or "").strip() != ADMIN_PANEL_PASSWORD:
...
PROMPT_CONFIG_DISPLAY_OVERRIDES = {
    ...
    STORYBOARD_REASONING_PROMPT_KEY: {
        "name": "故事板Sora：推理提示词前缀",
        "sort_order": 130,
    },
}
...
class ShotResponse(BaseModel):
    ...
    reasoning_prompt_status: str = "idle"
...
class BatchGenerateStoryboardReasoningPromptsRequest(BaseModel):
    ...
    for shot in shots:
        try:
            ...
            shot_dict = {
                ...
                'reasoning_prompt_status': getattr(shot, 'reasoning_prompt_status', "idle") or "idle",
...
def _build_storyboard_reasoning_request_data(
    db: Session,
    *,
    shot: models.StoryboardShot,
    episode: models.Episode,
    ...
    prompt_config = db.query(models.PromptConfig).filter(
        models.PromptConfig.key == STORYBOARD_REASONING_PROMPT_KEY,
        models.PromptConfig.is_active == True,
    ).first()
    prompt_template = str(getattr(prompt_config, "content", "") or "").strip() or STORYBOARD_REASONING_PROMPT_DEFAULT
    try:
        prompt = prompt_template.format(
            script_excerpt=excerpt,
            safe_duration=15,
    ...
    task_payload = {
        ...
        "prompt_key": STORYBOARD_REASONING_PROMPT_KEY,
    }
    return request_data, task_payload
...
def _submit_storyboard_reasoning_task(
    db: Session,
    *,
    shot: models.StoryboardShot,
    episode: models.Episode,
    ...
    request_data, task_payload = _build_storyboard_reasoning_request_data(
        db,
        shot=shot,
        episode=episode,
        script=script,
    ...
    return submit_and_persist_text_task(
        ...
        owner_type="shot",
        owner_id=int(shot.id),
        stage_key=STORYBOARD_REASONING_PROMPT_KEY,
        function_key="video_prompt",
        request_payload=request_data,
        task_payload=task_payload,
    )
...
def _queue_single_storyboard_reasoning_generation(
    shot_id: int,
    user: models.User,
    db: Session,
):
    ...
    shot.reasoning_prompt_status = "generating"
    try:
        _submit_storyboard_reasoning_task(
            db,
            shot=shot,
            episode=episode,
            script=script,
    ...
    return {
        "message": "推理提示词生成任务已开始，请稍后刷新页面查看结果。",
        "shot_id": shot_id,
    }
...
@app.post("/api/shots/{shot_id}/generate-reasoning-prompt")
async def generate_storyboard_reasoning_prompt(
    shot_id: int,
    user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """生成推理提示词并回填原剧本段落（后台任务）"""
    return _queue_single_storyboard_reasoning_generation(
        shot_id=shot_id,
        user=user,
        db=db,
    )
...
@app.post("/api/episodes/{episode_id}/batch-generate-storyboard-reasoning-prompts")
async def batch_generate_storyboard_reasoning_prompts(
    episode_id: int,
    request: BatchGenerateStoryboardReasoningPromptsRequest,
    user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """批量生成推理提示词并回填原剧本段落（后台任务）"""
    episode = db.query(models.Episode).filter(models.Episode.id == episode_id).first()
    if not episode:
        raise HTTPException(status_code=404, detail="片段不存在")
    ...
    shots = query.order_by(models.StoryboardShot.shot_number, models.StoryboardShot.variant_index).all()
    shots = [shot for shot in shots if str(shot.script_excerpt or "").strip()]
    if not shots:
        raise HTTPException(status_code=400, detail="没有选择有效的镜头")
    ...
    for shot in shots:
        try:
            ...
            shot.reasoning_prompt_status = "failed"
            print(f"[批量推理提示词提交失败] shot_id={shot.id} error={str(exc)}")

    db.commit()

    return {
        "message": f"批量推理提示词生成任务已提交，共 {submitted_count} 个镜头。",
