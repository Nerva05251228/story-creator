...
import json
import sys
from env_config import get_first_env, load_app_env, require_env

load_app_env()

API_BASE_URL = get_first_env("SCENE_IMAGE_API_BASE_URL", default="")
API_TOKEN = get_first_env("SCENE_IMAGE_API_TOKEN", default="")


def _scene_image_base_url() -> str:
    return require_env("SCENE_IMAGE_API_BASE_URL").rstrip("/")


def _scene_image_api_token() -> str:
    return require_env("SCENE_IMAGE_API_TOKEN")


def submit_image_generation(prompt, size="1:1", reference_images=None):
    """提交图片生成任务"""
    url = f"{_scene_image_base_url()}/images/generations"
    ...
    headers = {
        "Authorization": f"Bearer {_scene_image_api_token()}",
...
def check_task_status(task_id):
    """检查任务状态"""
    url = f"{_scene_image_base_url()}/tasks/{task_id}"
