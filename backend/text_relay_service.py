...
from ai_config import (
    ...
    get_relay_api_key,
    get_relay_base_url,
    get_relay_chat_completions_url,
    get_relay_models_url,
)
...
def _build_headers() -> Dict[str, str]:
    return {
        "Authorization": f"Bearer {get_relay_api_key()}",
        "Content-Type": "application/json",
    }


def _build_default_poll_url(task_id: str) -> str:
    return f"{get_relay_base_url()}/v1/tasks/{str(task_id or '').strip()}"
...
def _normalize_poll_url(poll_url: Optional[str], task_id: str) -> str:
    ...
    if normalized_poll_url.startswith("/"):
        base_parts = urlsplit(get_relay_base_url())
