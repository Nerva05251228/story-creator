...
async function showUserQuotaDialog() {
    ...
    try {
        const response = await apiRequest(`/api/video/quota/${encodedUsername}`);
        if (!response || !response.ok) {
            lastError = response ? `HTTP ${response.status}` : '请求失败';
        } else {
...
async function populateScriptSelectorOptions(selector, selectedValue = '') {
    if (!selector) return;
    try {
        const response = await apiRequest('/api/scripts/my');
        ...
        selector.innerHTML = `
            ...
            ${(Array.isArray(scripts) ? scripts : []).map(s => `<option value="${s.id}">${escapeHtml(s.name)}</option>`).join('')}
        `;
        if (selectedValue && Array.from(selector.options).some(option => option.value === selectedValue)) {
...
async function loadScriptStep() {
    ...
    if (isNewScript) {
        ...
        populateScriptSelectorOptions(selector);
...
async function loadStoryboardStep() {
    ...
    try {
        ...
        const storyboardCardsPromise = loadStoryboardCards();
        ...
        refreshStoryboardVideoUrlsInBackground(APP_STATE.currentEpisode);
...
async function refreshSelectedShotInBackground(shotId) {
    try {
        const response = await apiRequest(`/api/episodes/${APP_STATE.currentEpisode}/shots`);
        if (!response || !response.ok) return;
        const freshShots = await response.json();
        if (!Array.isArray(freshShots)) return;
        APP_STATE.shots = freshShots;
        if (!APP_STATE.currentShot || APP_STATE.currentShot.id !== shotId) return;
        const freshSelectedShot = freshShots.find(s => s.id === shotId);
        if (!freshSelectedShot) return;
        APP_STATE.currentShot = freshSelectedShot;
        renderStoryboardShotsGrid();
        renderStoryboardSidebar();
        updateSceneOverrideFromSelection();
    } catch (error) {
        console.error('Failed to refresh selected shot:', error);
    }
}

async function selectShot(shotId) {
    const localShot = APP_STATE.shots.find(s => s.id === shotId);
    if (!localShot) {
        await refreshSelectedShotInBackground(shotId);
        return;
    }

    APP_STATE.currentShot = localShot;
    APP_STATE.currentShotVideos = null;
    saveAppState();
    renderStoryboardShotsGrid();
    renderStoryboardSidebar();
    updateSceneOverrideFromSelection();
    refreshSelectedShotInBackground(shotId);
}
...
async function refreshStoryboardVideoUrlsInBackground(episodeId) {
    try {
        const refreshResponse = await apiRequest(`/api/episodes/${episodeId}/refresh-videos`, {
            method: 'POST'
        });
        if (!refreshResponse || !refreshResponse.ok) return;
        const refreshResult = await refreshResponse.json();
        if (!refreshResult || Number(refreshResult.updated_count || 0) <= 0) return;
        if (APP_STATE.currentEpisode !== episodeId || APP_STATE.currentStep !== 4) return;
        const reloadResponse = await apiRequest(`/api/episodes/${episodeId}/shots`);
        if (!reloadResponse || !reloadResponse.ok) return;
        const refreshedShots = await reloadResponse.json();
        if (!Array.isArray(refreshedShots)) return;
        APP_STATE.shots = refreshedShots;
        if (APP_STATE.currentShot) {
            const currentShot = refreshedShots.find(shot => shot.id === APP_STATE.currentShot.id);
            if (currentShot) APP_STATE.currentShot = currentShot;
        }
        renderStoryboardShotsGrid();
        renderStoryboardSidebar();
    } catch (err) {
        console.error('[loadStoryboardStep] 视频后台刷新失败:', err);
    }
}
...
async function fetchProvidersStats() {
    ...
    try {
        ...
        const response = await apiRequest('/api/video/provider-stats');
